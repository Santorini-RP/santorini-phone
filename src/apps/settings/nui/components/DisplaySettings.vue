...
import FixedSubPageHeader from '@core/nui/components/FixedSubPageHeader.vue'
...
<template>
  <div class="h-full flex flex-col">
    <FixedSubPageHeader title="Display & Brightness" previous-title="Settings" />
    
    <div class="flex-1 overflow-y-auto no-scrollbar">
      <div class="p-4 space-y-6 text-black dark:text-white">
        <!-- Appearance Section -->
        <div>
          <h3 class="text-xs uppercase text-gray-500 dark:text-gray-400 font-semibold px-4 mb-2">Appearance</h3>
          <div class="bg-white dark:bg-ios-dark-card rounded-lg p-4">
          ...
          <div class="bg-white dark:bg-ios-dark-card rounded-lg mt-4">
            <div class="p-3 flex justify-between items-center">
              <span class="font-medium">Automatic</span>
              <button
                @click="settingsStore.toggleAutomaticTheme()"
                class="w-12 h-7 rounded-full transition-colors duration-200 flex items-center p-0.5"
                :class="settingsStore.automaticTheme ? 'bg-green-500' : 'bg-gray-200 dark:bg-gray-600'"
              >
                <span
                  class="w-6 h-6 bg-white rounded-full shadow-md transform transition-transform duration-200"
                  :class="{ 'translate-x-5': settingsStore.automaticTheme }"
                ></span>
              </button>
            </div>
          </div>
        ...
        <div>
          ...
          <div class="bg-white dark:bg-ios-dark-card rounded-lg p-3 flex items-center">
